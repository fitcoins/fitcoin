...
uint256 hashGenesisBlock("0x000003eb6dd714c3f2935c22fc7c796730292da007f319386c29e157c3910a23"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // fitcoin: starting difficulty is 1 / 2^12
...
{
    // fitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // fitcoin: 1 day
static const int64 nTargetSpacing = 60; // fitcoin: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // l440
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // fitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, fitcoin - DarkGravity v2, written by Evan Duffield - evan@fitcoins.net */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // fitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // fitcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x00000f7d9e358d6461a925e494025bc8cac3ed3255799328444d0926869a2766");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block        
        const char* pszTimestamp = "Wired 15/April/2014 Inside the Operating System Edward Snowden Used to Evade the NSA";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1397702655;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 28952632;

        if (fTestNet)
        {
            block.nTime    = 1397535795;
            block.nNonce   = 3862132774;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5fd14dcaa3ea0f12f4789a00189679a99b1bf8a954bf09fcc2c992933094fa7a"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("fitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("fitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static fitcoinMiner(CWallet *pwallet)
{
    printf("fitcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running fitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("fitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&fitcoinMiner, pwallet));
}
